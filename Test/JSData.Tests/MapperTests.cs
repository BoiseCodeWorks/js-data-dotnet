...
namespace JSData.Tests
{
	...
	{
		private readonly IQueryable<Board> _boards;
		private Dictionary<string, Dictionary<object, object>> _cache = new Dictionary<string, Dictionary<object, object>>();

		public MapperTests()
		{
			var dict = CreateBoards(10).ToDictionary<Board, object, object>(x => x.Id, x => x);
			_boards = dict.Values.OfType<Board>().AsQueryable();
			_cache["board"] = dict;
		}
		...
		public void FindAllReturnsAll()
		{
			...
			var expectedBoards = _boards.ToList();
			var adapter = new InMemoryAdapter(_cache);
			var store = new DataStore(adapter);
			store.Map<Board>("board");

			var actualBoards = store.FindAll("board").ToList();
